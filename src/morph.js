...
(function (p5ex, CCC) {
  ...
  const clover = () => {
    ...
    points.forEach((point, i) => {
      const angle = i * angleInterval;
      const factor = 0.5 * (p.abs(sin$1(4 * angle)) + p.abs(cos$1(2 * angle)));
      point.targetX = radius * cos$1(angle) * factor;
      point.targetY = radius * sin$1(angle) * factor;
    });
  };
  const ribbon = () => {
    ...
    points.forEach((point, i) => {
      const angle = i * angleInterval;
      const factor = sin$1(4 * angle) - sin$1(3 * angle);
      point.targetX = radius * cos$1(angle) * factor;
      point.targetY = radius * sin$1(angle) * factor;
    });
  };
  const cross_2 = () => {
    ...
    points.forEach((point, i) => {
      const angle = i * angleInterval;
      const factor = 2 + (i % 2 === 0 ? 1 : -1) * sin$1(6 * angle);
      point.targetX = radius * cos$1(angle) * factor;
      point.targetY = radius * sin$1(angle) * factor;
    });
  };
  const cross_3 = () => {
    ...
    points.forEach((point, i) => {
      const angle = i * angleInterval;
      const factor = 1 + 0.2 * sin$1(6 * (angle + (i % 3) * PI$1 / 9));
      point.targetX = radius * cos$1(angle) * factor;
      point.targetY = radius * sin$1(angle) * factor;
    });
  };
  const combo = () => {
    ...
    points.forEach((point, i) => {
      const angle = i * angleInterval;
			const radius = (i % 3 === 0 ? 240 : 90);
      const factor = (i % 3 === 0 ? p.abs(sin$1(angle * 3)) : (i % 3 === 1 ? 2 + sin$1(angle * 6) : 2 - sin$1(angle * 6)));
      point.targetX = radius * cos$1(angle) * factor;
      point.targetY = radius * sin$1(angle) * factor;
    });
  };
  ...
  const initialize = () => {
	...
	const ptnArray = [circle, fan, clover, ribbon, cross_2, cross_3, combo, diffuse];
	const shapeColorArray = [[255, 0, 0, 16], [255, 127, 39, 16], [34, 177, 76, 16],
													 [255, 115, 160, 16], [0, 0, 255, 16], [163, 73, 164, 16], [153, 197, 22, 16], [64, 16]];
	const PTN_SPAN = 90;
	const PTN_NUM = ptnArray.length;
	const TOTAL_SPAN = PTN_SPAN * PTN_NUM;
  const draw$1 = () => {
		const ptnIndex = p.floor((p.frameCount % TOTAL_SPAN) / PTN_SPAN);
		if(p.frameCount % PTN_SPAN === 0){
			ptnArray[ptnIndex]();
		}
    tick();
    ...
    p.blendMode(p.BLEND);
		p.fill(...shapeColorArray[ptnIndex]);
